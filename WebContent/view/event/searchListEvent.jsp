...
<c:if test="${AUTH_USER.id=='admin'}"> 
	<a href="${cPath}/event/write.do" class="btn btn-primary">글쓰기</a>
</c:if>
</div>
...
<form action="${cPath}/event/search.do?" method="get">
	...
	<tr>
		...
		<td><input type="search" name="searchText" id="searchText" placeholder="검색어를 입력하세요" maxlength="100"/></td>
		<td><input type="submit" class="btn btn-outline-primary" value="검색"/></td>
	</tr>
	</table>
</form>
	...
	<nav aria-label="Page navigation" >
		<ul class="pagination" style="margin:10 10 10 10 ">
			<c:if test="${eventPage.startPage>5}">
				<li class="page-item"><a class="page-link" href="${cPath}/event/seach.do?pageNo=${eventPage.startPage-5}">Previous</a></li>
			</c:if>
			<c:forEach var="pNo" begin="${eventPage.startPage}" end="${eventPage.endPage}" step="1">
				<li class="page-item"><a class="page-link" href="${cPath}/event/seach.do?pageNo=${pNo}">${pNo}</a></li>
			</c:forEach>
			<c:if test="${eventPage.endPage<eventPage.totalPages}">
				<li class="page-item"><a class="page-link" href="${cPath}/event/seach.do?pageNo=${eventPage.startPage+5}">Next</a></li>
			</c:if>
		</ul>
	</nav>
