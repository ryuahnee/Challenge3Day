    ...
    <div class="container">
      ...
      <div>
        ...
        <form action="${cPath}/chall/search.do?" method="get">
          ...
          <c:if test="${AUTH_USER.id != null}"> <a href="${cPath}/chall/write.do" class="btn btn-outline-primary" style="float: right">챌린지 등록하기</a></c:if>
        </form>
    ...
    <nav aria-label="Page navigation">
      <ul class="pagination justify-content-center" style="margin:0 0">
        <c:if test="${challPage.startPage>5}">
          <li class="page-item"><a class="page-link" href="${cPath}/chall/<c:choose><c:when test="${param.searchCon!=null}">/search.do</c:when><c:otherwise>cate.do</c:otherwise></c:choose>?pageNo=${challPage.startPage-5}&option=${option}&searchCon=${searchCon}&cate_no=${cate_no}">이전</a></li>
        </c:if> 
        <c:forEach var="pNo"  begin="${challPage.startPage}" end="${challPage.endPage}" step="1">
          <li class="page-item"><a class="page-link" href="${cPath}/chall/<c:choose><c:when test="${param.searchCon!=null}">search.do</c:when><c:otherwise>cate.do</c:otherwise></c:choose>?pageNo=${pNo}&option=${option}&searchCon=${searchCon}&cate_no=${cate_no}">${pNo}</a></li>
        </c:forEach>
        <c:if test="${challPage.endPage< challPage.totalPages}"> 
          <li class="page-item"><a class="page-link" href="${cPath}/chall/<c:choose><c:when test="${param.searchCon!=null}">search.do</c:when><c:otherwise>cate.do</c:otherwise></c:choose>?pageNo=${challPage.startPage+5}&option=${option}&searchCon=${searchCon}&cate_no=${cate_no}">다음</a></li>
        </c:if> 
      </ul> 	
    </nav>
 </div> <!-- container -->
