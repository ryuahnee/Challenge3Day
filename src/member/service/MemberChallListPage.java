...
public class MemberChallListPage {
	private int total;				//총게시글수
	private int currentPage;		//보고싶은 페이지=>현재 페이지
	private List<MemberChallList> content;  //article목록
	private int totalPages;		//총페이지수   15
	private int startPage;			//시작페이지   12345  678910   11~15
	private int endPage;			//끝페이지
	
	MemberChallListPage(int total, int currentPage, int size, List<MemberChallList> content) {
		this.total=total;
	...
	public boolean hasNoChallenges() {
		return total==0;
	}

	//p650 58라인
	public List<MemberChallList> getContent() {
